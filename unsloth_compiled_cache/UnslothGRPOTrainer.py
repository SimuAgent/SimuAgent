...
from trl.trainer.grpo_trainer import (Any, AutoModelForCausalLM, AutoModelForSequenceClassification, AutoTokenizer, Dataset, GRPOConfig, GRPOTrainer, GenerationConfig, GuidedDecodingParams, IterableDataset, LLM, Optional, PeftConfig, PreTrainedModel, PreTrainedTokenizerBase, RepeatRandomSampler, RewardFunc, Sampler, SamplingParams, SyncRefModelCallback, Trainer, TrainerCallback, Union, apply_chat_template, broadcast_object_list, contextlib, create_reference_model, defaultdict, gather, gather_object, generate_model_card, get_comet_experiment_url, is_conversational, is_deepspeed_zero3_enabled, is_peft_available, is_peft_model, is_rich_available, is_wandb_available, maybe_apply_chat_template, nn, os, pad, patch, prepare_deepspeed, print_prompt_completions_sample, profiling_context, profiling_decorator, set_seed, textwrap, torch, transformers, unwrap_model_for_generation, version, wandb, warnings, Any, LLM, Union, apply_chat_template, broadcast_object_list, gather, gather_object, is_conversational, is_rich_available, maybe_apply_chat_template, nn, os, pad, print_prompt_completions_sample, profiling_context, torch, unwrap_model_for_generation, wandb, warnings, os, torch, transformers, GRPOTrainer, Trainer, gather, os, torch)
    ...
    def __init__(
        ...
        if self.use_vllm:
            self.llm = model.vllm_engine; self._last_loaded_step = 0; self.sampling_params = SamplingParams(
                    ...
                    guided_decoding=GuidedDecodingParams(backend="outlines", regex=args.vllm_guided_decoding_regex) if getattr(args, "vllm_guided_decoding_regex", None) is not None else None,
                    n=args.num_generations,
                    temperature=args.temperature,
                    top_p=args.top_p,
                    top_k=-1 if args.top_k is None else args.top_k,
                    min_p=0.0 if args.min_p is None else args.min_p,
                    repetition_penalty=args.repetition_penalty,**getattr(getattr(args, 'vllm_sampling_params', vLLMSamplingParams()), '_set_kwargs', {}),)
        else:
