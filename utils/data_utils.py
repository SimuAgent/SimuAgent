...
def get_preprocess_fn(name: str) -> Callable[[Dict], Dict]:
    ...
    elif 'CompleteSystem' in name:
        def preprocess_complete_system(x: Dict[str, Any]) -> Dict[str, Any]:
            return {
                "question": x["question"],
                "init_code": x["init_code"],
                "task": "simuenv"
            }
        return preprocess_complete_system
    else:
...
def format_dataset(dataset: Dataset,
    ...
    return dataset.map(lambda x: {
        ...
        "answer": x[answer_key] if answer_key in x else None
    }, num_proc=10)